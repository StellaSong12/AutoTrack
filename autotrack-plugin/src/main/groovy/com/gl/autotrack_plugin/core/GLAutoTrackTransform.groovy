...
class GLAutoTrackTransform extends Transform {
    private GLAutoTrackTransformHelper transformHelper
    private WaitableExecutor waitableExecutor
    private URLClassLoader urlClassLoader
    private String glHookSdkJarPath // “com.netease.glhook.GLDataAutoTrackHelper” 类所在路径
    private volatile boolean isFoundSDKJar = false
    private boolean isProjectLibrary = false

    GLAutoTrackTransform(GLAutoTrackTransformHelper transformHelper, boolean isProjectLibrary) {
        this.transformHelper = transformHelper
        this.isProjectLibrary = isProjectLibrary
        if (!transformHelper.disableGlAutoTrackMultiThread) {
    ...
    boolean isIncremental() {
        return !transformHelper.disableGlAutoTrackIncremental
    }
    ...
    private void checkHookSDK() {
        try {
            Class sdkClazz = urlClassLoader.loadClass(GLAutoTrackHookConfig.GL_AutoTrack_API.replace("/", "."))
            ProtectionDomain pd = sdkClazz.getProtectionDomain()
            CodeSource cs = pd.getCodeSource()
            glHookSdkJarPath = cs.getLocation().toURI().getPath()
        } catch (ClassNotFoundException ignored) {
            throw new IllegalStateException("未检测到 Autotrack SDK")
        } catch (Throwable ignored) {
            Logger.warn("Can not load find Autotrack SDK jar's path.")
        ...
        if (isIncremental) {
            ...
            switch (status) {
                ...
                case Status.CHANGED:
                    ...
                    transformJar(destFile, jarInput, destFile, context)
                    break
                case Status.REMOVED:
                    Logger.info("jar status = $status:$destFile.absolutePath")
                    if (destFile.exists()) {
    ...
    void transformJar(File dest, JarInput jarInput, File destFile, Context context) {
        ...
        if (!transformHelper.extension.disableJar || checkJarValidate(jarInput)) {
            ...
            Logger.info("结束遍历 jar：" + jarInput.file.absolutePath + " " + destFile.getPath())
        }
        if (modifiedJar == null) {
    ...
    private boolean checkJarValidate(JarInput jarInput) {
        try {
            if (isFoundSDKJar || glHookSdkJarPath == null) {
                return false
            }
            def jarLocation = jarInput.file.toURI().getPath()
            if (glHookSdkJarPath.length() == jarLocation.length() && glHookSdkJarPath == jarLocation) {
                isFoundSDKJar = true
                return true
            } else {
                return false
        ...
        while (enumeration.hasMoreElements()) {
            ...
            if (entryName.endsWith(".DSA") || entryName.endsWith(".SF")) {
                ...
                try {
                    ...
                    sourceClassBytes = GLAutoTrackUtil.toByteArrayAndAutoCloseStream(inputStream)
                } catch (Exception e) {
     ...
     * 真正修改类中方法字节码
     */
    private byte[] modifyClass(byte[] srcClass, ClassNameAutoTrack classNameAutoTrack) {
        try {
            ClassWriter classWriter = new ClassWriter(ClassWriter.COMPUTE_MAXS)
            ClassVisitor classVisitor = new GLAutoTrackClassVisitor(classWriter, classNameAutoTrack, transformHelper)
